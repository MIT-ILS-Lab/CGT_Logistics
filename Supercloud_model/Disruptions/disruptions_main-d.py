...
model.DISRUPT = Param(model.p, within=Binary)
...
def obj_rule(model):
    ...
    sum((model.DISRUPT[p]*(10476+9312)) for p in model.p)
...
def C1_rule(model,p):
    ...
    return model.CTM[p] == sum((model.E1[m]*(model.CIM[m]+model.CVM[m]))*len(model.t)/len(model.p) for m in model.m) + model.DELAY[p]*model.MANUPEN + \
        model.DISRUPT[p]*sum((model.E1[m]*(model.CIM[m]+model.CVM[m]))*len(model.t)/len(model.p) for m in model.m)
...
#C2
def C2_rule(model,p):
    #return model.TTC[p] == sum(model.Y1[p,c,m,j,t]*model.U1[c,m,j] for c in model.c for m in model.m for j in model.j for t in model.t) + \
       # sum(model.Y3[p,m,h,j,t]*model.U3[m,h,j] for m in model.m for h in model.h for j in model.j for t in model.t)
    return model.TTC[p] == sum(model.Y1[p,c,m,j,t]*model.U1[c,m,j] for c in model.c for m in model.m for j in model.j for t in model.t) + \
        sum(model.Y1[p,c,m,j,t]*model.U1[c,m,j] for c in model.c for m in model.m for j in model.j for t in model.t)*model.DISRUPT[p] + \
        sum(model.Y3[p,m,h,j,t]*model.U3[m,h,j] for m in model.m for h in model.h for j in model.j for t in model.t)
...
def MSB1_rule(model,p,c,t,tt):
    if tt == t + model.TLS*(1+model.DISRUPT[p]) :
...
def MSB2_rule(model,p,m,t,tt):
    if tt == t + model.TMFE*(1+model.DISRUPT[p]) + model.DELAY[p]:
...
def MSB3_rule(model,p,c,m,j,t,tt):
    if tt == t + model.TT1[j]*(1+model.DISRUPT[p]):
...
if __name__ == '__main__':
    ...
    run_results.to_csv(f'{date}_disruptions_d_{n}.csv')
    manu_config.to_csv(f'{date}_disruptions_d_manu_{n}.csv')
    end_time = time.time()
